...
// letter in the alphabet within the string; ie: b occurs no more times than a; c occurs no more times than b; 
// etc. Note that letter a has no previous letter.
...
bool isBeautifulString(string inputString) {
    std::map<char, int> mapCF;
    for (char itr = 'a'; itr != 'z' + 1; itr++) {
        mapCF[itr] = 0;
    }
    ...
    for (itr = mapCF.begin(); itr != mapCF.end(); itr++) {
