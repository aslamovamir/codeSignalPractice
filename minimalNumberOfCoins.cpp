...
// coins[i] is divisible by coins[i - 1]. Find the minimal number of banana-coins you'll have to spend to buy a banana given 
// the banana's price.
// Example
...
int solution(vector<int> coins, int price) {
    int coins_used = 0;
    int index_runner = coins.size()-1;
    while(price != 0) {
        coins_used += price/coins[index_runner];
        price = price%coins[index_runner];
        index_runner--;
    }
    return coins_used;
}
